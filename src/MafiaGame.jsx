...
const MOBILE_NAV = [["home","Home"],["dispatch","Dispatch"],["city","City"],["combat","Combat"]];
...
function Overview({ p, go }) {
  ...
  return <><section className="living-hero"><Skyline/><div className="living-hero-copy"><small>BLACKWOOD CITY · LIVE</small><h1>Good evening, <em>{p.name}.</em></h1><p>The city remembers who shows up. Make today count.</p><button className="primary" onClick={() => go("hustles")}>Keep earning <b>→</b></button></div><div className="city-weather"><i/><span><small>CITY STATUS</small><b>Business is moving</b></span></div></section>
    <button className="dispatch-promo" onClick={() => go("dispatch")}><span><small>NEW · BLACKWOOD DISPATCH</small><b>Know your next move.</b><em>One live board for campaign goals, district operations and faction progress.</em></span><strong>Open Dispatch <i>→</i></strong></button><div className="overview-grid"><Panel title="City business" eyebrow="WHAT NEEDS ATTENTION"><button className="feature-order" onClick={() => go("family")}><i>M</i><span><small>NEW FAMILY ORDER</small><b>The Harbor Collection</b><em>Don Salvatore wants the docks secured before midnight.</em></span><strong>›</strong></button><div className="quick-grid">{[["hustles", "↻", "Street Work", "No energy required"], ["crimes", "♠", "Crimes", `${p.nerve} nerve ready`], ["gym", "▲", "Train", `${p.energy} energy ready`], ["market", "◇", "Market", "Real player listings"]].map(x => <button key={x[0]} onClick={() => go(x[0])}><i>{x[1]}</i><span><b>{x[2]}</b><small>{x[3]}</small></span><em>›</em></button>)}</div></Panel>
...
export default function MafiaGame({ initialPlayer = null, character = null, user = null, onPlayerChange = null, onSignOut = null, onDeleteAccount = null }) {
  ...
  const content = serverPages.includes(page) ? <CityCoreHub initialTab={page} user={user} onState={syncCore} /> : { home: <Overview p={p} go={navigate} />, dispatch: <SeasonHub onNavigate={navigate} />, city: <City go={navigate} />, family: <CommunityHub user={user} initialTab="families" />, chat: <CommunityHub user={user} initialTab="chat" />, players: <CommunityHub user={user} initialTab="players" />, rankings: <CommunityHub user={user} initialTab="rankings" />, economy: <EconomyHub onWalletChange={syncWallet}/>, arcade: <CasinoHub onWalletChange={syncWallet}/>, safety: <SafetyHub onDeleteAccount={onDeleteAccount}/> }[page];
