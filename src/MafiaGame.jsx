...
const NAV = [["home", "Overview"], ["crimes", "Crimes"], ["hustles", "Street Work"], ["combat", "Attack"], ["gym", "Gym"], ["work", "Jobs"], ["missions", "Missions"], ["economy", "Economy"], ["arcade", "Casino"], ["city", "City"], ["shop", "Shops"], ["market", "Player Market"], ["bank", "Bank"], ["hospital", "Hospital"], ["jail", "Jail"], ["property", "Properties"], ["family", "Family"], ["chat", "World Chat"], ["players", "Players"], ["social", "Contacts"], ["mail", "Messages"], ["forums", "Forums"], ["rankings", "Rankings"], ["awards", "Awards"], ["inventory", "Inventory"]];
const PLACES = [
  ["economy", "Federal Trust FX", "TRADING FLOOR", "Trade six currency pairs and earn a banking career offer.", "economy"],
  ["arcade", "Rossi's Casino", "GAMES & WAGERS", "Blackjack, slots and European roulette behind the green door.", "arcade"],
  ["hustles", "Street Work", "OPEN ALL NIGHT", "No-energy jobs with mastery, loot and soft heat limits.", "hustles"],
  ["market", "Blackwood Exchange", "PLAYER MARKET", "Buy and sell equipment listed by real players.", "market"],
...
function Overview({ p, go }) {
  ...
  return <><PageHead eyebrow="BLACKWOOD CITY" title={<>Good evening, <em>{p.name}.</em></>} text="The city remembers who shows up. Make today count."><button className="primary" onClick={() => go("hustles")}>Keep earning <b>→</b></button></PageHead>
...
function City({ go }) { return <><PageHead eyebrow="BLACKWOOD DIRECTORY" title="The City" text="Every block has an opportunity, if you know which door to knock on." /><div className="city-grid">{PLACES.map(([id, name, kicker, desc, icon]) => <button key={id} onClick={() => go({ hospital: "hospital", bank: "bank", dealer: "shop", estate: "property" }[id] || id)}><i>{["economy","arcade"].includes(icon) ? <GameIcon name={icon}/> : icon}</i><span><small>{kicker}</small><b>{name}</b><em>{desc}</em></span><strong>›</strong></button>)}</div></>; }
...
export default function MafiaGame({ initialPlayer = null, character = null, user = null, onPlayerChange = null, onSignOut = null }) {
  ...
  return <div className="game-shell"><header className="topbar"><button className="hamb" onClick={() => setMenu(!menu)} aria-label="Open navigation">☰</button><button className="brand" onClick={() => setPage("home")}><i>M</i><span><b>MORETTI</b><small>BLACKWOOD CITY</small></span></button><div className="resources" data-tutorial="resources"><Resource label="Energy" value={p.energy} max={p.maxEnergy} tone="energy" icon="⚡" /><Resource label="Nerve" value={p.nerve} max={p.maxNerve} tone="nerve" icon="♦" /><Resource label="Health" value={p.health} max={p.maxHealth} tone="health" icon="+" /><Resource label="Happy" value={p.happy} max={p.maxHappy} tone="happy" icon="♥" /></div><button className="cash"><small>ON HAND</small><b>{money(p.cash)}</b></button><button className="avatar" onClick={() => setAccountOpen(!accountOpen)} aria-label="Account menu">{initials}</button>{accountOpen && <div className="account-menu"><small>SIGNED IN</small><b>{character?.codename || p.name}</b><em>{user?.email}</em><button onClick={onSignOut}>Sign out</button></div>}</header>
    <aside className={`sidebar ${menu ? "open" : ""}`}><div className="profile"><i>VM<span>{p.level}</span></i><div><b>{p.name}</b><small>{p.title}</small><figure><i style={{ width: `${p.xp / xpNeed(p.level) * 100}%` }} /></figure><em>{p.xp} / {xpNeed(p.level)} XP</em></div></div><nav>{NAV.map(([id, label]) => <button data-page={id} className={page === id ? "active" : ""} key={id} onClick={() => { setPage(id); setMenu(false); }}><i><GameIcon name={id}/></i>{label}</button>)}</nav><button className="market-link" onClick={() => setPage("economy")}><i><GameIcon name="economy"/></i><span><b>Federal Trust FX</b><small>Six markets open</small></span><em>›</em></button><blockquote>“Keep your friends close. Keep your ledger closer.”<small>— Don Salvatore</small></blockquote></aside>
    <main>{content}</main>{notice && <div className={`toast ${notice.tone}`}><i>{notice.tone === "bad" ? "!" : "✓"}</i>{notice.text}</div>}<GuidedTutorial step={p.tutorialStep} done={p.tutorialDone} onNavigate={next => { setPage(next); setMenu(false); }} onState={syncCore}/><AdviserPanel onNavigate={next=>{setPage(next);setMenu(false)}} /></div>;
}
