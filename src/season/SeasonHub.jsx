...
const SEASONS = [
  ["Harbor Lights", "The docks are open late. Build a record the whole city can verify."],
  ["Ledger Run", "Clean decisions move faster than loud promises in Financial Ward."],
  ["Northside Line", "Keep the supply routes open and earn trust where it matters."],
  ["The Long Week", "Every district is watching. Turn consistent work into a lasting name."],
];
...
function Objective({ icon, label, detail, value, target, onClick, complete = false }) {
  return <article className={`dispatch-objective ${complete ? "complete" : ""}`}>
    ...
    <div><small>{complete ? "COMPLETE" : "SEASON TARGET"}</small><b>{label}</b><span>{detail}</span><Meter value={value} target={target} /></div>
...
export default function SeasonHub({ onNavigate }) {
  ...
  const season = useMemo(() => {
    ...
    const index = Math.max(0, Math.floor((Date.now() - start) / (1000 * 60 * 60 * 24 * 28))) % SEASONS.length;
    return { number: index + 1, name: SEASONS[index][0], description: SEASONS[index][1] };
  }, []);
  ...
  const seasonScore = Math.min(100, Math.round((Math.min(clears, 6) / 6 * 35) + (Math.min(assignments, 8) / 8 * 25) + (Math.min(wins, 3) / 3 * 20) + (nextMission ? 0 : 20)));
  const firstAction = nextMission ? {
    ...
    text: "No energy is required. Clear a marked NPC crew and keep building your season record.", action: "Open operations", page: "operations", tone: "copper",
  };
  ...
  return <div className="dispatch-page">
    <header className="dispatch-hero">
      <div className="dispatch-hero-copy"><small>BLACKWOOD DISPATCH · SEASON {String(season.number).padStart(2, "0")}</small><h1>{season.name}</h1><p>{season.description}</p><div className="dispatch-hero-actions"><button className="dispatch-primary" onClick={() => onNavigate(firstAction.page)}>{firstAction.action}<span>→</span></button><button className="dispatch-secondary" onClick={() => onNavigate("missions")}>View campaign</button></div></div>
      <div className="dispatch-score"><div className="dispatch-ring" style={{ "--progress": `${seasonScore * 3.6}deg` }}><span><b>{seasonScore}%</b><small>SEASON<br />PROGRESS</small></span></div><small>SERVER VERIFIED</small><em>{syncedAt ? `Synced ${syncedAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Live record"}</em></div>
    </header>
