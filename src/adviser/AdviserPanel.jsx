...
export default function AdviserPanel({ onNavigate, open, onClose, page = 'home' }) {
  ...
  return <Dialog label="Ask Adviser" onClose={onClose} className="bw-adviser">
    ...
    <div className="bw-adviser-body"><p className="bw-adviser-intro">A little direction goes a long way. Ask about your record, your next move, or how a system works.</p><div className="bw-prompts">{['What should I do next?','Explain '+pageLabel(page)+' and how I can use it.','How can I play without energy?'].map(text => <button key={text} disabled={busy} onClick={() => ask(text)}>{text}</button>)}</div>
