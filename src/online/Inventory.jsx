...
export const SETS = [
  ...
  { id: "crimson-oni", name: "Crimson Oni", atlas: "/assets/loot-v2/crimson-oni.webp", pieces: ["Oni Cleaver", "Demon Kabuto", "Oni Warplate", "Demon Greaves"], two: "+12 Attack", four: "+18 Attack" },
  ...
  { id: "chrome-wraith", name: "Chrome Wraith", atlas: "/assets/loot-v2/chrome-wraith.webp", pieces: ["Wraith Spear", "Chrome Skull", "Piston Cage", "Mercury Boots"], two: "+8 Defense", four: "+25 Max HP" },
...
export function getArmoryBonuses(value, profile = {}) {
  ...
  if (role === "samurai") { totals.attack = Math.round(totals.attack * 1.1); totals.hp += 15; }
  if (role === "ghost") { totals.speed = Math.round(totals.speed * 1.1); totals.crit += 5; }
  if (role === "netrunner") { totals.tech = Math.round(totals.tech * 1.1); totals.xp += 8; }
...
export default function Inventory({ profile, value, onChange, onClose, onStartRun = null, onCompleteRun = null, onSaveLoadout = null, onEnhanceItem = null }) {
  const inventory = normalizeInventory(value);
  ...
  return <main className="inventory-v2">
    ...
    {tab === "runs" && <section className="runs-v2"><div className="run-zone"><small>SHIBUYA UNDERCITY</small><h2>District Sweep</h2>{running ? <><p className="run-brief">Move with the left pad. Tap ATTACK and time DASH to survive three escalating waves.</p><Brawl stats={{ hp: 110 + (totals.defense || 0) * 2, maxHp: 110 + (totals.defense || 0) * 2, str: 12 + (totals.attack || 0), def: 6 + (totals.defense || 0), spd: 8 + (totals.speed || 0), dex: 8 + (totals.tech || 0), crit: 2, wPow: 0, aPow: 0 }} enemy={{ id: "punk", name: "Undercity Sweep", kanji: "掃", lvl: Math.max(1, Math.floor(score / 18) + 1), hp: 70 + score * 2, atk: 12 + score * .22 }} onEnd={finishRun}/></> : <><p>Fight through hostile blocks and extract one equipment drop. Prismatic gear has a 0.1% base chance with a guaranteed pity drop at 1,000 dry runs.</p>{inventory.runs === 0 && <div className="tutorial-callout"><b>FIRST CONTRACT</b><span>Guaranteed Green weapon + 12 Nano Shards. Equip it, then enhance it to +1.</span></div>}<div className="drop-rates">{RARITY_ORDER.map((rarity) => <span key={rarity} style={{ "--tier": RARITIES[rarity].color }}><i/>{RARITIES[rarity].label}<b>{RARITIES[rarity].weight}%</b></span>)}</div><button className="run-action" onClick={startRun}>Enter District</button></>}<div className="run-stats"><span>RUNS <b>{inventory.runs}</b></span><span>PRISM PITY <b>{inventory.prismPity}/1000</b></span><span>VAULT <b>{inventory.owned.length}/200</b></span></div></div><aside className="run-history"><h3>Recent extractions</h3>{!(inventory.history || []).length && <p>No run data yet.</p>}{(inventory.history || []).map((entry, index) => { const item = byId(entry.id); return item && <div key={`${entry.at}-${index}`}><ItemArt item={item} small/><span><b>{item.name}</b><small>{entry.duplicate ? `Duplicate · +${RARITIES[item.rarity].shard} shards` : "New equipment"}</small></span></div>;})}</aside></section>}
