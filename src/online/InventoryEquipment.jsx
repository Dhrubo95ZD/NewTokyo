...
function Stats({ item }) {
  const stats = [["ATK", item.effectiveAttack ?? item.attack], ["DEF", item.effectiveDefense ?? item.defense], ["SPD", item.effectiveSpeed ?? item.speed], ["DEX", item.effectiveDexterity ?? item.dexterity]].filter(([, value]) => Number(value) > 0);
