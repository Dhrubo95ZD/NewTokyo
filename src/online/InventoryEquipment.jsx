...
const icon = kind => kind === "weapon" ? "†" : kind === "armor" ? "◇" : kind === "accessory" ? "◆" : "+";
...
export function InventoryEquipment({ inventory, loadout, level, busy, onAction }) {
  ...
  return <div className="equipment-page" data-tutorial="equipment">
    <section className="loadout-panel">
      ...
      <div className="slot-grid">{SLOTS.map(([slot, label, help]) => { const item = equipped[slot]; return <article className={item ? `filled ${item.rarity}` : ""} key={slot}><i>{item ? icon(item.kind) : "○"}</i><div><small>{label}</small><b>{item?.name || "Empty slot"}</b><p>{item ? `${item.rarity} · level ${item.level_required}` : help}</p></div>{item && <button disabled={busy} onClick={() => onAction("bw_unequip_slot", { p_slot: slot }, `${item.name} unequipped.`)}>Remove</button>}</article>; })}</div>
    </section>
...
export function ItemShop({ items, cash, level, busy, onBuy }) {
  ...
  const pageSize = 24;
  const filtered = useMemo(() => items.filter(item => (slot === "all" || item.slot === slot || item.kind === slot) && item.name.toLowerCase().includes(query.toLowerCase())), [items, query, slot]);
