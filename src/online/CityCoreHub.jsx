...
import CrimeBoard from "../ui/CrimeBoard.jsx";
...
export default function CityCoreHub({ initialTab, progressionTab = "story", user, onState }) {
  ...
  const [pendingCrime, setPendingCrime] = useState(null), [crimeResult, setCrimeResult] = useState(null);
  ...
  const load = useCallback(async () => {
    try {
      const [stateResult, loadoutResult] = await Promise.all([supabase.rpc("bw_get_state"), supabase.rpc("bw_get_loadout")]);
      if (stateResult.error) throw new Error(stateResult.error.message);
      setError(""); acceptState(stateResult.data);
      if (!loadoutResult.error) setLoadout(loadoutResult.data || { equipment: [], bonuses: {} });
    } catch (problem) { setError(problem.message || "Could not connect. Please retry."); }
  }, [acceptState]);
  ...
  const act = async (rpc, params = {}, success = "City record updated.", loader) => {
    if (actionLock.current) return;
    actionLock.current = true; setBusy(true); setError(""); setNotice("");
    const crimeId = rpc === "bw_do_crime" ? params.p_crime_id : null;
    setPendingCrime(crimeId);
    try {
      const { data: value, error: actionError } = await supabase.rpc(rpc, params);
      if (actionError || value?.error) throw new Error(actionError?.message || value.error);
      acceptState(value);
      if (crimeId && typeof value?.event?.success === "boolean") setCrimeResult({ ...value.event, crimeId, sequence:Date.now() });
      else setNotice(success);
      if (loader) await loader(value);
    } catch (problem) { setError(problem.message || "The connection was interrupted. Refresh your record before retrying."); }
    finally { actionLock.current = false; setBusy(false); setPendingCrime(null); }
  };
  ...
  if (error && !data) return <><PageHeading heading={heading} /><section className="core-shell"><Empty title="Could not open city records" text={error} /><button className="core-primary retry" onClick={load}>Retry connection</button></section></>;
  ...
  return <><PageHeading heading={heading} status={player.status} />{error && <div className="core-alert bad" role="alert">{error}<button onClick={() => setError("")}>×</button></div>}{notice && <div className="core-alert good" role="status">{notice}<button onClick={() => setNotice("")}>×</button></div>}<section className="core-shell">
    {initialTab === "crimes" && <CrimeBoard data={data} busy={busy} pendingCrime={pendingCrime} result={crimeResult} onAttempt={crime => act("bw_do_crime", { p_crime_id:crime.id }, "Crime attempt recorded.")}/>}
