...
export default function MafiaAccount() {
  ...
  useEffect(() => {
    ...
    (async () => { const { data, error } = await supabase.from("player_saves").select("save_data").eq("user_id", user.id).maybeSingle(); if (cancelled) return; if (error) setStatus(error.message); const remote = data?.save_data || {}; const foundCharacter = remote.character || remote.characterProfile || null; if (foundCharacter) await supabase.from("profiles").upsert({ id: user.id, display_name: foundCharacter.codename || user.user_metadata?.name || "Associate", avatar_url: user.user_metadata?.avatar_url || user.user_metadata?.picture || null, last_seen_at: new Date().toISOString() }); setSave(remote); setCharacter(foundCharacter); setPlayer(foundCharacter ? normalizeCloudPlayer(remote, user) : null); setLoading(false); })();
  ...
  const createCharacter = async profile => { setLoading(true); const role = roles.find(item => item.id === profile.role); const seed = { ...INITIAL, name: profile.codename, title: role.name, ...Object.fromEntries(Object.entries(role.bonus).map(([key, value]) => [key, INITIAL[key] + value])) }; const next = { schemaVersion: 4, core: { ...seed, money: seed.cash }, character: profile, meta: { createdAt: Date.now(), updatedAt: Date.now() } }; const [{ error: profileError }, { error: saveError }] = await Promise.all([supabase.from("profiles").upsert({ id: user.id, display_name: profile.codename, avatar_url: user.user_metadata?.avatar_url || user.user_metadata?.picture || null, last_seen_at: new Date().toISOString() }), supabase.from("player_saves").upsert({ user_id: user.id, save_data: next })]); if (profileError || saveError) { setStatus((profileError || saveError).message); setLoading(false); return; } setSave(next); setCharacter(profile); setPlayer(seed); setLoading(false); };
  const persistPlayer = useCallback(nextPlayer => { if (!user) return; clearTimeout(saveTimer.current); saveTimer.current = setTimeout(async () => { const next = { schemaVersion: 4, core: { ...nextPlayer, money: nextPlayer.cash, onlineUserId: user.id }, character, meta: { updatedAt: Date.now() } }; await supabase.from("player_saves").upsert({ user_id: user.id, save_data: next }); await supabase.rpc("sync_my_leaderboard"); await supabase.from("profiles").update({ last_seen_at: new Date().toISOString() }).eq("id", user.id); }, 650); }, [user?.id, character]);
