...
export default function MafiaAccount() {
  ...
  const persistPlayer = useCallback(nextPlayer => { if (!user) return; clearTimeout(saveTimer.current); saveTimer.current = setTimeout(async () => { const next = { schemaVersion: 4, core: { ...nextPlayer, money: nextPlayer.cash, onlineUserId: user.id }, character, meta: { updatedAt: Date.now() } }; await supabase.from("player_saves").upsert({ user_id: user.id, save_data: next }); }, 650); }, [user?.id, character]);
