...
export default function HomeBoard({ p, go, onState, skyline }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([supabase.rpc('bw_get_state'), supabase.rpc('bw_operations_snapshot'), supabase.rpc('bw_connected_progression_snapshot'), supabase.rpc('bw_daily_life_snapshot')]).then(results => {
      if (!alive) return;
