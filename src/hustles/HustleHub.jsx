...
export default function HustleHub({onState}) {
  ...
  return <div className="hustle-hub">{error&&<div className="market-alert bad">{error}</div>}{event&&<div className="hustle-result"><i>✓</i><span><small>{event.name}</small><b>+{money(event.cash)} · +{event.xp} XP</b>{event.itemName&&<em>Found {event.itemName}</em>}</span></div>}
    ...
    <div className="hustle-grid">{data.activities.map(activity=><article key={activity.id}><header><i><GameIcon name="hustles"/></i><span><small>{activity.district}</small><b>{activity.name}</b></span></header><p>{activity.description}</p><dl><div><dt>Base cash</dt><dd>{money(activity.cashMin)}–{money(activity.cashMax)}</dd></div><div><dt>Heat</dt><dd>+{activity.heatGain}</dd></div><div><dt>Loot chance</dt><dd>{activity.lootChance}%</dd></div></dl><button disabled={busy||cooldown>0} onClick={()=>run(activity)}>{busy?"Working contact…":cooldown?`Next contact · ${cooldown}s`:"Run hustle · 0 energy"}</button></article>)}</div>
