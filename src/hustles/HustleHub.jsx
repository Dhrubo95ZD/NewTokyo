...
export default function HustleHub({onState}) {
  ...
  return <div className="hustle-hub">{error&&<div className="market-alert bad">{error}</div>}{event&&<div className="hustle-result"><i>✓</i><span><small>{event.name}</small><b>+{money(event.cash)} · +{event.xp} XP</b>{event.itemName&&<em>Found {event.itemName}</em>}</span></div>}
    <header className="hustle-status"><div><small>NO ENERGY REQUIRED</small><h2>Street Work</h2><p>Keep earning whenever energy and nerve are empty. Mastery always rises; heat reduces efficiency, and the server allows 240 runs per rolling 24 hours.</p></div><div className="hustle-numbers"><span><small>24H RUNS</small><b>{profile.dailyRuns} / 240</b></span><span><small>MASTERY</small><b>{profile.mastery}</b></span><span><small>LIFETIME TAKE</small><b>{money(profile.totalCash)}</b></span></div></header>
    <section className="heat-board"><div><span><small>CITY HEAT</small><b>{Number(profile.heat).toFixed(1)} / 100</b></span><em>Heat cools by 1 point each minute</em></div><figure><i style={{width:`${profile.heat}%`}}/></figure><footer><span>Current reward efficiency</span><b>{Math.round(profile.rewardMultiplier*100)}%</b><em>Never below 25% · 240 runs / 24h</em></footer></section>
