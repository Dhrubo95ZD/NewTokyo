...
export default function CasinoHub({ onLedgerChange = null, onCashChange = null }) {
  ...
  const netProfit=Number(state?.netArcadeProfit || 0);
  const redeem=()=>call("bw_redeem_arcade_winnings",{p_credits:Math.max(100,redeemAmount),p_request_id:crypto.randomUUID()});

  return <div className="casino-page"><header><small>ROSSI'S RECREATION HALL</small><h1>The Arcade</h1><p>Blackjack, slots and roulette use play-earned Arcade Dollars from the same wallet as Prime FX. Every result is recorded by the city server.</p><div className="virtual-currency-notice"><b>ARCADE DOLLARS · $</b><span>Shared with Prime FX · cannot be bought with city cash or real money · no cash-to-Arcade conversion · verified net wins may redeem one-way into ordinary in-game cash</span></div></header>{error&&<div className="casino-error">{error}</div>}
    ...
    <section className={`casino-table ${tab}-room`}><div className="casino-balance"><small>AVAILABLE ARCADE DOLLARS</small><b>{arcadeMoney(state?.balance)}</b><div className="arcade-ledger-line"><small className="arcade-redeemable">Redeemable net: {arcadeMoney(redeemable)}</small><small className={netProfit >= 0 ? "arcade-profit" : "arcade-loss"}>Lifetime P/L: {netProfit >= 0 ? "+" : ""}{arcadeMoney(netProfit)}</small></div><small className="arcade-rate">100 Arcade Dollars = $50 city cash · verified net wins only · losses reduce the current redeemable net</small><div className="arcade-redemption"><label><span>REDEEM ARCADE WINNINGS</span><input type="number" min="100" max={Math.max(100,redeemable)} step="100" value={redeemAmount} onChange={event=>setRedeemAmount(Math.max(100,Number(event.target.value)))} /></label><button disabled={busy||redeemable<100||redeemAmount>redeemable} onClick={redeem}>{busy?"Settling…":"Redeem to city cash"}</button></div></div>
