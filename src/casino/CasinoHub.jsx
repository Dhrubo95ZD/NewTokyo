...
export default function CasinoHub({ onWalletChange }) {
  ...
  const accept=value=>{setState(value);if(Number.isFinite(Number(value?.cashBalance)))onWalletChange?.(Number(value.cashBalance));};
  ...
  return <div className="casino-page"><header><small>ROSSI'S RECREATION HALL</small><h1>The Arcade</h1><p>Blackjack, slots and roulette use a separate play-earned Arcade Dollar wallet. Results are recorded by the city server.</p><div className="virtual-currency-notice"><b>ARCADE DOLLARS · $</b><span>Separate wallet · cannot be bought with city cash or real money · winnings may be redeemed one-way into ordinary city cash</span></div></header>{error&&<div className="casino-error">{error}</div>}
