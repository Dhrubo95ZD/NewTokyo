...
function Slots({state,spinning}) {
  ...
  return <div className={`slots ${spinning?"spinning":""}`}><div className="slot-cabinet"><header><small>ROSSI'S ORIGINAL</small><h2>The Three Kings</h2><div className="jackpot-lights" aria-hidden="true">{Array.from({length:11},(_,index)=><i key={index}/>)}</div></header><div className="reels">{symbols.map((symbol,index)=><div className="reel-window" key={index}><div className="reel-strip" style={{"--reel":index}}>{[symbol,"BAR","◆","7","🍒",symbol].map((entry,item)=><i key={item}>{slotGlyph(entry)}</i>)}</div><span className="payline"/></div>)}</div><footer><span>🍒 🍒 pays 2×</span><b>♛ 7 ◆ BAR</b><span>Three match wins</span></footer></div><p className={result?.payout>0?"casino-win":""}>{spinning?"Reels in motion…":result?.message||"Match three symbols on the gold payline. Three sevens pay the top prize."}</p></div>;
}
