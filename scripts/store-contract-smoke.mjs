...
for (const marker of ["bw-supporter-entry", "CASH SHOP · GOOGLE PLAY", "Open cash shop", "go('store')"]) assert.ok(contents.home.includes(marker), `home cash-shop entry missing: ${marker}`);
...
assert.ok(!contents.plugin.includes("connect(null);"), "billing plugin must not start a background connection during load");
for (const marker of ["connectionInFlight", "pendingConnections", "finishConnection"]) assert.ok(contents.plugin.includes(marker), `billing connection guard missing: ${marker}`);
