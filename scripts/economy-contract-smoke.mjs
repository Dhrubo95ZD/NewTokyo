...
if(!game.includes('["economy", "Economy", "商"]'))throw new Error("Economy is not a top-level destination");
for(const token of ['onOpenEconomy?.("auction")','Auction House','Crafting','Life Skills'])if(!game.includes(token))throw new Error(`Economy destination is missing ${token}`);
