...
const art = await stat(new URL("../public/assets/characters/runner-equipment-v3.webp", import.meta.url));
...
assert.ok(styles.includes('grid-template-areas:"model model" "weapon helmet" "armor boots" "megachip megachip"'), "mobile slots must form an ordered, non-overlapping matrix");
assert.ok(styles.includes(".slot-orbit{display:contents}"), "equipment cards must participate in the stage grid");
assert.ok(ui.includes("fully helmeted android equipment preview"), "loadout must use the fully helmeted android render");
